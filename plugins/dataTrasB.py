...
#             print(rf)
    ...
    s= year+"-"+da[1]+"-"+da[0]
    fecha =datetime.datetime.strptime("{0}".format(s), "%Y-%b-%d").strftime("%Y-%m-%d")
    return fecha
...
def data_transform(data):
    ...
    df = pd.read_csv(data)
    ds = pd.Series(['university','career','inscription_date','first_name','last_name','gender','age','postal_code','location','email'])

#     ds = pd.DataFrame(['university','career','inscription_date','first_name','last_name','gender','age','postal_code','location','email'])

    ds = ['university','career','inscription_date','first_name','last_name','gender','age','postal_code','location','email']
